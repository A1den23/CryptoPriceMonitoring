...
class ConfigManagerRegressionTests(unittest.TestCase):
    def test_config_manager_reads_stablecoin_depeg_settings(self) -> None:
        ...
        self.assertTrue(config.stablecoin.depeg_monitor_enabled)
        self.assertEqual(config.stablecoin.depeg_top_n, 12)
        self.assertEqual(config.stablecoin.universe_cache_path, "/app/data/custom-top25.json")
        self.assertFalse(config.stablecoin.universe_auto_refresh_enabled)
    ...
    def test_config_manager_reads_runtime_notification_and_heartbeat_settings(self) -> None:
        ...
        self.assertEqual(config.telegram.bot_token, "token-from-config")
        self.assertEqual(config.telegram.chat_id, "chat-from-config")
        self.assertEqual(config.runtime.bot_heartbeat_file, "/tmp/custom-bot-heartbeat")
        self.assertEqual(config.runtime.bot_heartbeat_interval_seconds, 45.0)
        self.assertEqual(config.runtime.monitor_heartbeat_file, "/tmp/custom-monitor-heartbeat")
...
class DockerRegressionTests(unittest.TestCase):
    ...
    def test_healthcheck_resolves_configured_heartbeat_paths(self) -> None:
        ...
        with patch.dict(
            ...
            self.assertEqual(
                healthcheck._resolve_heartbeat_path(b"python -m monitor"),
                Path("/tmp/custom-monitor-heartbeat"),
            )
            self.assertEqual(
                healthcheck._resolve_heartbeat_path(b"python -m bot"),
                Path("/tmp/custom-bot-heartbeat"),
            )

    def test_healthcheck_keeps_default_heartbeat_paths(self) -> None:
        import scripts.healthcheck as healthcheck

        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(
                healthcheck._resolve_heartbeat_path(b"python -m monitor"),
                Path("/tmp/monitor_heartbeat"),
            )
            self.assertEqual(
                healthcheck._resolve_heartbeat_path(b"python -m bot"),
                Path("/tmp/bot_heartbeat"),
            )
